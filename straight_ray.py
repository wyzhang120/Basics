...
def ray_seg_len(xzsrc, xzrec, nx, nz, dx=1., dz=1.,
    ...
    xz = xz[:id_max+1]
    # xz = np.vstack((xzsrc, xz))
    # ray_len = np.vstack((np.zeros([1, 1]), ray_len.reshape([-1, 1])))
    dxz = np.array((dx, dz))
